...
- **識別子は対象外。** `Test_check_輸入したケース` のように、消す類型が関数名へ焼き込まれていても
  本スキルは検出しない。気づいたら **補遺** で述べるに留め、finding にはしない。
...
finding にならないが整理役へ伝えるべきこと —— 識別子へ焼き込まれた類型、他スキルが所管する
観点、このファイル自身の欠け。**無ければこの節ごと省く。** 判定内訳の件数には数えない。

```text
## 補遺

- <1行ずつ。判定を持たないので、場所と何に気づいたかだけを書く>
```
