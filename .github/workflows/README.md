...
**`push` 側の `branches:` は、宣言が指す workflow では生成される。** 指していない workflow が
これを持っていれば落ちる —— どちらであっても、手で書いた値がそのまま残ることはない。

生成器は行を書き換えるので、**扱える記法はブロック形式（ハイフンで項目を並べる形）だけ**である。
フロースタイル（`branches: [a, b]`）、キーの行末コメント、`branches-ignore:` はいずれも
エラーになる。読み飛ばさないのが目的なので、読めない形は「無い」ではなく「読めない」として
落とす。
