...
## 提供段には version manager を残していないため（ADR-0023 決定11）、コンテナ経由では成立しない。
##
## CI でも ADR-0023 決定7 のとおり実行環境へ直接用意するので、同じ経路で動く。
##
...
tool-cooldown-gate:
	...
	@$(TOOL_COOLDOWN_ENV) $(call RUN_SCRIPT_HOST,tool-cooldown,gate --base=$(BASE))

.PHONY: tool-cooldown-audit ## 宣言全件の cooldown 状況を棚卸し（警告のみ・ゲートしない）
## バイパスの期限切れだけは失敗させる。**期限は宣言が変わらなくても訪れる**ため、
## このターゲットは定期実行にも載せる（ADR-0020 決定18）。
tool-cooldown-audit:
	@$(TOOL_COOLDOWN_ENV) $(call RUN_SCRIPT_HOST,tool-cooldown,audit)
