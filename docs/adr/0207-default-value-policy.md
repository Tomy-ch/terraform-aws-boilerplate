# ADR-0207: 設定値は公式ベストプラクティスを根拠に決定し、利用者へ公開しない

- Status: Accepted
- Date: 2026-09-15
- Scope: repository-wide
- Related: ADR-0201, ADR-0202, ADR-0203, ADR-0206, ADR-0301, ADR-0401
...
15. 設定値の決定時には、以下をboilerplate内部の記録として残す（ADR-0206 決定8と同一の記録へまとめてよい）。
...
17. 公式推奨の変化に伴うdefault値の変更は、公開契約を変えない限り破壊的変更としない（ADR-0202 決定5）。ただし、適用時にresource置換を伴う場合はADR-0202 決定8に従う。

## 検討した代替案

### 案A: 設定値を可能な限り利用者へ公開する

利用者の制御範囲は最大になるが、公開variableが増加し、誤設定経路が増える。ADR-0101・ADR-0201と整合しない。
...
移行しやすいが、上書き経路が既定の利用形態になった時点で保証が失われる（ADR-0101 案B、ADR-0204と同じ帰結）。
