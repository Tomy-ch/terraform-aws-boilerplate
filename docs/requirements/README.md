...
**このディレクトリのファイルが正本である。** 要件を練る下書きがリポジトリの外にあってもよいが、根拠として参照してよいのはここにあるものだけとする。

Issue と Pull Request は `v0.2.md` の節番号を根拠に書かれている。外の下書きだけを直しても、その根拠は変わらない。外で固めた内容は、Pull Request としてここへ入れる。
