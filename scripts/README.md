...
|`branches/`|分岐のパターンを、単一の宣言（[`lib/branches`](lib/branches/branches.go)）から [`branch-protection.json`](../.github/settings/branch-protection.json) の `conditions.ref_name.include` と、workflow の `on.push.branches` およびブランチ集合の式（`fromJSON`）へ反映し、ずれを検出する（ADR-0603 決定1-3）。実行時に読む側（`base-branch` / `release` / `repo-setup`）は同じパッケージを import するので、生成も突合も要らずコンパイラが一致を保証する。`apply` は書き換える**区間だけ**を置き換える: JSON 全体を再直列化するとキーの並びが辞書順へ変わり、YAML を往復させるとコメントも引用の仕方も失われ、どちらも触るべきでない箇所まで書き換わる。逆に検証を省いて文字列置換だけで済ませると、整形が変わった瞬間に別の配列の閉じ括弧へ着地する。走査は**全 workflow に掛ける** —— 宣言に載っている分だけを開くと、対象外の workflow が直接書いたパターンは誰の目にも触れない。`github.base_ref == '<branch>'` のように生成では追随できない形は、書き換えず落とす。|`make branches-apply` / `branches-check`|
