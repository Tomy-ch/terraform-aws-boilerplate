...
|Script|Description|Invoked By|
|---|---|---|
|`skill-lint/`|`.claude/skills/**` の Markdown が inline code span で名指しした `make` ターゲット・`/<skill>`・パスが実在するかを検査する。**スキルは検査を持たない散文であり、名指しした先が消えても何も赤くならない** —— 嘘をついたスキルは、読んだ人が叩くまで嘘だと分からず、叩いた人は「この手順は古い」ではなく「自分の環境が壊れている」と読む。例示と参照はコードフェンスの位置で分ける（`lib/mdscan`）。フェンスの外の inline code span は実在するものを名指しする、という契約。**行単位で検査を黙らせる口を持たない** —— 通らない行を通すためのマーカは溜まり、対象が消えても残る。存在しないものを述べたい文（`how-to` の「在って当然に見えるが存在しない」ターゲットなど）は、コードスパンに入れずに書く。パス参照は**先頭セグメントがリポジトリ直下に実在するものだけ**を対象にする。未配線の領域（`modules/` / `examples/`）への言及はずれではなく予告であり、その領域が作られた日に検査が自動で始まる。|`make skill-lint`|
...
|`pin-runners/`|workflow の `runs-on:` が名指しする GitHub-hosted runner の label を `.github/runners-pin.toml`（宣言の正本）へ固定する。`ubuntu-latest` のような浮動の label が指す OS は GitHub の都合で入れ替わり、入れ替わりはこちらの履歴に現れないので、別の OS で検査するようになったことに 何かが壊れるまで気づけない（[ADR-0505](../docs/adr/0505-ci-runner-image-pinning.md)）。`apply` は宣言の浮動 label を固定先へ書き換え、行末の注記は保つ。`check` は同じ判定を書き換えなしで実行し drift で落ちる。**宣言のどちらの側にも無い label はエラーである** —— 列・`group:`・`${{ }}` の形もここへ来る。読めない形を取りこぼしとして通すと、固定されていない job が黙って残る。宣言が0件、走査した `runs-on:` が0件のときも同じくエラーで、「固定されている」と「何も見なかった」を分ける。ブロックスカラーの中身は見ない（判定は `lib/yamlblock` が1箇所で持つ）。版を上げる手順は宣言を書き換えて `apply` することで、そのとき diff に出るのは移行そのものだけになる。|`make pin-runners-apply` / `pin-runners-check`|
...
|`versions/`|`mise.toml` の `[tools]` が宣言する版を、それを書き写している箇所 —— `docker/tools/Dockerfile` の `FROM golang:` 2件と `FROM node:` 1件・ベースイメージとの照合値2件（[0503](../docs/adr/0503-tool-execution-form.md) 決定3）・`npm install` へ渡す版2件、`scripts/go.mod` の `go` ディレクティブ、`.makefiles/host-tools.mk` が焼き込む `mise install "<名前>@<版>"` 2件 —— へ反映し、ずれを検出する（ADR-0501 決定19 が版の正本を単一 manifest と定める）。**宣言の読み取りは `lib/misetoml` が持ち、ここが決めるのはどのキーを要求するかだけである。** 照合値をシェル変数として焼き込むのは、`ARG` だと `--build-arg` で外から差し替えられ、照合しているふりをした検査になるためである。**ずれてもビルドは通る**ので、この検査が無いと「ずれている」と「揃っている」が緑で区別できない —— 落ちるのは、ずれた版に無い機能を使ったときだけである。一致件数は下限ではなく**厳密な件数**で見る: 写しが増えたことも減ったことも、宣言との対応が崩れた合図である。`FROM` の照合はコメント行を除き、レジストリを前置した `FROM docker.io/library/golang:` は別物として掴まない。`mise install` の照合は閉じ引用符を錨に取り、**レシピ行の前置き（`\t@`）を捕獲群の中に入れる** —— 群の外で消費すると、一致件数を変えないまま置換でその2文字が消える。全ての rule を検証し終えるまで1バイトも書かない。|`make versions-apply` / `versions-check`|
