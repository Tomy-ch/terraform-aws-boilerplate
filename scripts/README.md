...
|`branches/`|保護対象のブランチ・リリース線の形・ブランチ名の接頭辞を、単一の宣言（[`.github/branches.toml`](../.github/branches.toml)）から生成先へ反映し、ずれを検出する（ADR-0603 決定1-3）。`apply` は保護設定の `conditions.ref_name.include` と、宣言が名指しした workflow の `on.push.branches` を組み直す。`check` は書き換えずに同じ比較を行い、ずれで非ゼロ終了する。**生成できない読み手は実行時に読む** —— `list <集合>` と `get <キー>` がその口で、`base-branch` / `release` / `repo-setup` は `lib/branches` 経由で同じ宣言を読む。閉じる方向で落ちる: 宣言に無い workflow が `on.push.branches` を持っていること、宣言が指す workflow が実在しないこと、解釈できない行、存在しない集合の参照 —— いずれもエラーで、読み飛ばさない。読み飛ばすと、打ち間違えた1行が「宣言されていない」と同じ扱いになり、保護対象から黙って消える。|`make branches-apply` / `branches-check` / `branches-list SET=` / `branches-get KEY=`|
