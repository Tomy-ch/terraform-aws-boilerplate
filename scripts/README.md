...
|`branches/`|保護対象のブランチを、単一の宣言（[`lib/branches`](lib/branches/branches.go)）から [`branch-protection.json`](../.github/settings/branch-protection.json) へ反映し、ずれを検出する（ADR-0603 決定1-3）。**生成できるのはここだけである** —— 他の読み手（`base-branch` / `release` / `repo-setup`）は同じパッケージを import するので、生成も突合も要らずコンパイラが一致を保証する。`apply` は `conditions.ref_name.include` の**区間だけ**を置き換える: 全体を再直列化するとキーの並びが辞書順へ変わり、触るべきでない箇所まで書き換わる。逆に検証を省いて文字列置換だけで済ませると、整形が変わった瞬間に別の配列の閉じ括弧へ着地する。だから JSON として検証したうえで括弧の対応から区間を決める。**workflow の `on.push.branches` は生成しない** —— そちらは required context を報告せず、絞っても merge を塞がない。|`make branches-apply` / `branches-check`|
