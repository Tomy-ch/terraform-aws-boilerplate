...
func branchCreationSteps(existing []string) ([]step, []string) {
	...
	steps := make([]step, 0, len(branches.Deploy))
	skipped := make([]string, 0)

	for _, b := range branches.Deploy {
		if have[b] {
...
func defaultBranchStep(repo string) step {
	return step{name: "gh", args: []string{"api", "-X", "PATCH", "repos/" + repo, "-f", "default_branch=" + branches.Default}}
}

// isReleaseBranch は、初期化時に破棄してよいリリースブランチかを返します。
// 判定は前方一致ではなく部分一致（hotfix/release/... のような名前も対象に含める）。
// 接頭辞は scripts/lib/branches が持つ。
func isReleaseBranch(name string) bool {
	return strings.Contains(name, branches.ReleasePrefix)
}
...
func createBranches(r runner) error {
	...
	existing := make([]string, 0, len(branches.Deploy))
...
func moveDefaultBranch(r runner) error {
	...
	if err := r.run(step{name: "git", args: []string{"switch", branches.Default}}); err != nil {
		return err
	}
