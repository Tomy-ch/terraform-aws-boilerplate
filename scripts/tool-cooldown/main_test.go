...
// pyRequirementsFixture は python/*.in の形を最小で再現する。
const pyRequirementsFixture = `# 直接宣言
graphifyy[sql]==0.9.25
`

// pyLockFixture は python/*.txt（uv pip compile --generate-hashes の出力）の形を最小で再現する。
// 継続行と注釈行を残してあるのは、宣言の行だけを読めていることを確かめるため。
const pyLockFixture = `# This file was autogenerated by uv via the following command:
#    uv pip compile python/graphify.in --generate-hashes --universal -o python/graphify.txt
graphifyy==0.9.25 \
    --hash=sha256:e902205873d129e9c76c11fea4268480042603590290ed600707354e74314c0c
    # via -r python/graphify.in
networkx==3.6.1 \
...
func Test_parsePyRequirements(t *testing.T) {
	t.Parallel()
	...
	t.Run("正常系", func(t *testing.T) {
		...
		t.Run("継続行を持つ lockfile の形でも宣言の行だけを読む", func(t *testing.T) {
			t.Parallel()
			got, err := parsePyRequirements([]byte(pyLockFixture))
			require.NoError(t, err)
			require.Len(t, got, 2)
			assert.Equal(t, "pypi:graphifyy@0.9.25", got[0].id())
			assert.Equal(t, "pypi:networkx@3.6.1", got[1].id())
		})
	})
	...
	t.Run("異常系", func(t *testing.T) {
		...
		t.Run("読み切れない行はエラーにする", func(t *testing.T) {
			t.Parallel()
			_, err := parsePyRequirements([]byte(strings.Repeat("a", bufio.MaxScanTokenSize+1) + "\n"))
			require.Error(t, err)
		})
	})
}
...
func Test_parseDeclarations(t *testing.T) {
	...
	t.Run("正常系", func(t *testing.T) {
		...
		t.Run("パスで形式を振り分けて 1 つの一覧へまとめる", func(t *testing.T) {
			t.Parallel()
			got, err := parseDeclarations([]declaration{
				{path: miseFile, content: []byte(miseOneAquaTool)},
				{path: "python/graphify.in", content: []byte(pyRequirementsFixture)},
			})
			require.NoError(t, err)
			require.Len(t, got, 2)
		...
		t.Run("宣言元のパスをツールに持たせる", func(t *testing.T) {
			...
			require.NoError(t, err)
			require.Len(t, got, 1)
			assert.Equal(t, "python/graphify.in", got[0].file)
		})
	})
	...
	t.Run("異常系", func(t *testing.T) {
		...
		t.Run("解析に失敗したファイルのパスを添えてエラーにする", func(t *testing.T) {
			...
			_, err := parseDeclarations([]declaration{
				{path: "python/graphify.in", content: []byte(strings.Repeat("a", bufio.MaxScanTokenSize+1) + "\n")},
			})
			require.Error(t, err)
			assert.Contains(t, err.Error(), "python/graphify.in")
		})
	})
}
...
func Test_declarationPaths(t *testing.T) {
	...
	t.Run("正常系", func(t *testing.T) {
		// glob を取り違えると、PyPI の宣言だけが検査されないまま通る。
		//nolint:paralleltest // 親がプロセス共有の状態を差し替えるため並列化不可
		t.Run("mise.toml と python/*.in を列挙する", func(t *testing.T) {
			dir := t.TempDir()
			require.NoError(t, os.MkdirAll(filepath.Join(dir, "python"), 0o750))
			for _, name := range []string{"python/b.in", "python/a.in", "python/a.txt"} {
				require.NoError(t, os.WriteFile(filepath.Join(dir, name), []byte(""), 0o600))
			}
			t.Chdir(dir)

			got, err := declarationPaths()
			require.NoError(t, err)
			assert.Equal(t, []string{miseFile, "python/a.in", "python/b.in"}, got)
		})

		//nolint:paralleltest // 親がプロセス共有の状態を差し替えるため並列化不可
		t.Run("python/*.in が無ければ mise.toml だけを返す", func(t *testing.T) {
			t.Chdir(t.TempDir())

			got, err := declarationPaths()
			require.NoError(t, err)
			assert.Equal(t, []string{miseFile}, got)
		})
	})
}
...
func Test_backendKind(t *testing.T) {
	...
	t.Run("正常系", func(t *testing.T) {
		...
		// pipx と uvx はどちらも PyPI を指すため、同じ種別へ寄せる。
		t.Run("pipx は PyPI 系に分類する", func(t *testing.T) {
			t.Parallel()
			assert.Equal(t, "pypi", backendKind("pipx:sqlfluff"))
		})

		t.Run("uvx は PyPI 系に分類する", func(t *testing.T) {
			t.Parallel()
			assert.Equal(t, "pypi", backendKind("uvx:sqlfluff"))
		})
...
func Test_windowFor(t *testing.T) {
	...
	t.Run("正常系", func(t *testing.T) {
		...
		t.Run("PyPI の窓は 7 日", func(t *testing.T) {
			t.Parallel()
			assert.Equal(t, registryWindowDays, windowFor("pipx:sqlfluff"))
		})
...
func Test_validateBypasses(t *testing.T) {
	...
	t.Run("異常系", func(t *testing.T) {
		...
		t.Run("宣言に無いエントリは違反にする", func(t *testing.T) {
			...
			require.Len(t, violations, 1)
			assert.Contains(t, violations[0].msg, "どちらにも存在しません")
		})
	})
}
...
func Test_verifyLocks(t *testing.T) {
	...
	// writePair は *.in と *.txt の組を書き、*.in のパスを返します。
	writePair := func(t *testing.T, lock string) string {
		t.Helper()
		dir := t.TempDir()
		req := filepath.Join(dir, "graphify"+pyRequirementSuffix)
		require.NoError(t, os.WriteFile(req, []byte(pyRequirementsFixture), 0o600))
		if lock != "" {
			require.NoError(t, os.WriteFile(filepath.Join(dir, "graphify"+pyLockSuffix), []byte(lock), 0o600))
		}
		return req
	}
	...
	t.Run("正常系", func(t *testing.T) {
		...
		t.Run("宣言と lockfile が同じ版を指していれば違反にしない", func(t *testing.T) {
			t.Parallel()
			got := verifyLocks([]tool{{key: "pypi:graphifyy[sql]", version: "0.9.25", file: writePair(t, pyLockFixture)}})
			assert.Empty(t, got)
		})

		// mise が解決するツールに lockfile は無い。全件を見に行くと、宣言の大半が違反に化ける。
		t.Run("mise.toml 由来のツールは見ない", func(t *testing.T) {
			t.Parallel()
			got := verifyLocks([]tool{{key: "aqua:owner/repo", version: "1.2.3", file: miseFile}})
			assert.Empty(t, got)
		})
	})
	...
	t.Run("異常系", func(t *testing.T) {
		...
		t.Run("lockfile が別の版を固定していれば違反にする", func(t *testing.T) {
			...
			got := verifyLocks([]tool{{key: "pypi:graphifyy[sql]", version: "0.9.25", file: req}})
			require.Len(t, got, 1)
			assert.Equal(t, req, got[0].file)
			assert.Contains(t, got[0].msg, "固定しているのは graphifyy==0.9.24")
		})

		t.Run("lockfile に宣言が無ければ違反にする", func(t *testing.T) {
			t.Parallel()
			req := writePair(t, "networkx==3.6.1\n")

			got := verifyLocks([]tool{{key: "pypi:graphifyy[sql]", version: "0.9.25", file: req}})
			require.Len(t, got, 1)
			assert.Contains(t, got[0].msg, "にありません")
		})

		t.Run("lockfile が無ければ違反にする", func(t *testing.T) {
			t.Parallel()
			req := writePair(t, "")

			got := verifyLocks([]tool{{key: "pypi:graphifyy[sql]", version: "0.9.25", file: req}})
			require.Len(t, got, 1)
			assert.Contains(t, got[0].msg, "読めません")
		})
	})
}

func Test_lockedVersion(t *testing.T) {
	t.Parallel()

	t.Run("正常系", func(t *testing.T) {
		t.Parallel()

		t.Run("名前が一致する行の版を返す", func(t *testing.T) {
			t.Parallel()
			got, ok := lockedVersion([]byte(pyLockFixture), "networkx")
			require.True(t, ok)
			assert.Equal(t, "3.6.1", got)
		})

		// PyPI の名前は表記が揺れても同じパッケージを指す。素の比較では取り違える。
		t.Run("表記の揺れを正規化して突き合わせる", func(t *testing.T) {
			t.Parallel()
			got, ok := lockedVersion([]byte("Foo_Bar==1.0.0\n"), "foo-bar")
			require.True(t, ok)
	...
	t.Run("異常系", func(t *testing.T) {
		...
		t.Run("固定されていない名前は見つからないものとして返す", func(t *testing.T) {
			t.Parallel()
			_, ok := lockedVersion([]byte(pyLockFixture), "sqlfluff")
			assert.False(t, ok)
		})
	})
}

func Test_pyPackageName(t *testing.T) {
	t.Parallel()

	t.Run("正常系", func(t *testing.T) {
		t.Parallel()

		// extras は付随依存の選択でしかなく、パッケージの同一性には関わらない。
		t.Run("backend の接頭辞と extras を落とす", func(t *testing.T) {
			t.Parallel()
			assert.Equal(t, "graphifyy", pyPackageName("pypi:graphifyy[sql]"))
		})

		t.Run("大文字と区切り記号を正規化する", func(t *testing.T) {
			t.Parallel()
			assert.Equal(t, "foo-bar", pyPackageName("pypi:Foo_.Bar"))
		})

		t.Run("接頭辞が無いキーもそのまま名前として扱う", func(t *testing.T) {
			t.Parallel()
			assert.Equal(t, "sqlfluff", pyPackageName("sqlfluff"))
		})
	})
}
...
func Test_pypiPackageAt(t *testing.T) {
	t.Parallel()

	t.Run("正常系", func(t *testing.T) {
		t.Parallel()

		t.Run("releases の先頭ファイルのアップロード時刻を返す", func(t *testing.T) {
			t.Parallel()
			client := fakeUpstream(t, respondJSON("/pypi/sqlfluff/json",
				`{"releases":{"3.5.0":[{"upload_time_iso_8601":"2026-06-01T00:00:00Z"}]}}`))

			at, err := pypiPackageAt(t.Context(), client, "sqlfluff", "3.5.0")
			require.NoError(t, err)
			assert.Equal(t, "2026-06-01T00:00:00Z", at.Format(time.RFC3339))
		})
	})
	...
	t.Run("異常系", func(t *testing.T) {
		...
		// ゼロ値の時刻を返すと公開から数十年経った扱いになり、窓を無条件に通過する。
		t.Run("releases に無いバージョンはエラーにする", func(t *testing.T) {
			t.Parallel()
			client := fakeUpstream(t, respondJSON("/pypi/sqlfluff/json",
				`{"releases":{"3.4.0":[{"upload_time_iso_8601":"2026-06-01T00:00:00Z"}]}}`))

			_, err := pypiPackageAt(t.Context(), client, "sqlfluff", "3.5.0")
			require.ErrorIs(t, err, errNotFound)
		})

		t.Run("ファイルが 1 つも無いバージョンはエラーにする", func(t *testing.T) {
			t.Parallel()
			client := fakeUpstream(t, respondJSON("/pypi/sqlfluff/json", `{"releases":{"3.5.0":[]}}`))
...
func Test_publishedAt(t *testing.T) {
	...
	t.Run("正常系", func(t *testing.T) {
		...
		// PyPI は extras を知らないため、付けたまま問い合わせると存在しないパッケージになる。
		t.Run("pipx は extras を落として PyPI を引く", func(t *testing.T) {
			t.Parallel()
			got := requestedPath(t, tool{key: "pipx:graphifyy[sql]", version: "0.9.25", backend: "pipx:graphifyy[sql]"})
			assert.Equal(t, "/pypi/graphifyy/json", got)
		})
	})
